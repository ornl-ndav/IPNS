...
public class Parameter {
  ...
  */
  ...
  public static void printParameters( Parameter[] params) {
    System.out.println(Parameter.getPrintParameters(params));
  ...
  public static void printParametersBrief( Parameter[] params) {
    System.out.println(Parameter.getPrintParametersBrief(params));
  }

  /**
   */
  public static String getPrintParameters( Parameter[] params) {
    StringBuffer temp = new StringBuffer();
    for ( int ii=0; ii < params.length; ii++) {
      temp.append( "  |-" + params[ii].type + "Parameter " + (ii+1) + 
		   "\n");
      temp.append( "  | |-" + "Name: " + params[ii].name.trim() + "\n");
      temp.append( "  | |-" + "Current Value: " + params[ii].value + "\n");
      temp.append( "  | |-" + "DB signal name: " + 
		   params[ii].dbsignal.trim() + "\n" );
      if ( params[ii].optionsAvailable ) {
	temp.append( "  | |-Options" + "\n");
	for (int jj=0; jj<(params[ii].options).length; jj++){
	  temp.append( "       | | |-" + params[ii].options[jj].trim() + 
		       "= " +
		       jj + "\n");
	}
      }
  ...
  /**
   */
  public static String getPrintParametersBrief( Parameter[] params) {
    StringBuffer temp = new StringBuffer();
    for ( int ii=0; ii < params.length; ii++) {
      temp.append( "  |-" + params[ii].name.trim() + ": " );
      if ( params[ii].optionsAvailable ) {
	temp.append( params[ii].options[(int)(params[ii].value)].trim() + "\n");
      }
      else {
	temp.append( params[ii].value + "\n");
      }
    }
    return temp.toString();
  }
